...
<#
.SYNOPSIS
    Save-MailAttachments.ps1 をタスクスケジューラへ登録する（5分間隔・現在ユーザー・ログオン時のみ）。

.DESCRIPTION
    schtasks.exe /Create を使い、Save-MailAttachments.ps1 を5分ごとに実行するタスクを登録する。
...
.NOTES
    ★このスクリプトは開発機では実行しないこと（現地PCでのみ実行する）。
    タスク名: MailAttachSaver
#>
...
$taskName = "MailAttachSaver"
$scriptPath = Join-Path $PSScriptRoot "Save-MailAttachments.ps1"

if (-not (Test-Path -LiteralPath $scriptPath)) {
    throw "Save-MailAttachments.ps1 が見つかりません: $scriptPath"
}
...
#   （実測確認済み）、ProcessStartInfo.Arguments に生のコマンドライン文字列を
#   直接渡す方式を使う。/TR の値に含まれる " は \" にエスケープしてから
#   外側を " で囲む（標準的なコマンドライン引数のエスケープ規則）。
# ------------------------------------------------------------
...
if ($createResult.ExitCode -eq 0) {
    ...
    Write-Host "確認方法: タスクスケジューラを開き「MailAttachSaver」を確認、または次を実行:"
    Write-Host ('  schtasks.exe /Query /TN "MailAttachSaver" /V /FO LIST')
}
else {
